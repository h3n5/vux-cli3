...
const parseVirtualComponent = require(VuxLoaderPath + 'libs/parse-virtual-component')
const parseXIcon = require(VuxLoaderPath + 'libs/parse-x-icon')
const removeTagCode = require(VuxLoaderPath + 'libs/getTag').removeTagCode
const reserveTagCode = require(VuxLoaderPath + 'libs/getTag').reserveTagCode
...
const getLocals = (options) => {
  const vuxLocalesContent = fs.readFileSync(path.join(__dirname, '../locales/all.yml'), 'utf-8')
  return yamlReader.load(vuxLocalesContent)
}
...
module.exports = function (source) {
  ...
  let isVuxVueFile = this.resourcePath.replace(/\\/g, '/').indexOf('src/components') > -1
